<x-layouts.app>
    ...
    <div class="mb-6 flex items-center text-sm">
        ...
        <a href="{{ route('admin.locations.index') }}" class="text-blue-600 dark:text-blue-400 hover:underline">{{ __('Locations') }}</a>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mx-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Location Details and Recent Activity') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">{{ __('Location Information') }}</h2>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Name') }}</label>
                            <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $location->name }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Address') }}</label>
                            <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $location->address }}</p>
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Radius') }}</label>
                            <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $location->radius }} {{ __('meters') }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Status') }}</label>
                            <span class="mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                {{ $location->is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400' }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Created') }}</label>
                                <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $location->created_at->format('M j, Y g:i A') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Last Updated') }}</label>
                                <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $location->updated_at->format('M j, Y g:i A') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Today') }}</span>
                            <span class="font-semibold text-gray-900 dark:text-gray-100">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="p-6 text-center">
                <div class="text-gray-500 dark:text-gray-400">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">{{ __('No Attendances Yet') }}</h3>
                    <p class="text-gray-600 dark:text-gray-400">{{ __('No attendance records have been created for this location.') }}</p>
                </div>
            </div>
        </div>
    </div>
